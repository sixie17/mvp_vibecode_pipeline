...
- **Filter**: fires when the issue's assignee becomes `LINEAR_BOT_USER_ID` (`linear/webhooks.py`'s `is_issue_assigned_to`), or when someone other than `LINEAR_BOT_USER_ID` posts a new comment (`is_new_human_comment`) on an issue that's already been refined
- **Implementation**: [linear/webhooks.py](linear/webhooks.py) (signature/event filtering), [linear/client.py](linear/client.py) (`LinearClient`, used for the deterministic verify/fail-comment path *and* for posting the refine/plan/failure comments — its issue query also fetches each comment's `id`/`createdAt`, not just `body`, for the clarification loop's idempotency check), [linear/mcp.py](linear/mcp.py) (`build_linear_mcp_client`, wraps `langchain_mcp_adapters.MultiServerMCPClient` against `LINEAR_MCP_URL`, Linear's read-only endpoint), [linear/services.py](linear/services.py) (`handle_issue_assigned`, `handle_ticket_comment`, `verify_github_integration`, `refine_ticket_agent` — a `langgraph.prebuilt.create_react_agent` bound to Linear's live read-only MCP tools) with its system prompt in [linear/prompts.py](linear/prompts.py) (`REFINE_AGENT_PROMPT`), exposed by [linear/views.py](linear/views.py) (`LinearWebhookView`). Step 5 itself is [planner/](planner/) — see "Dev plan generation" below.
- **Audit trail**: none locally, by design — the refined spec (initial and any revision), the dev plan, and any failure explanation are each a comment left on the Linear issue itself, and every agent run (LLM calls + tool calls) is traced in LangSmith. See [CLAUDE.md](CLAUDE.md#state-derived-not-stored).
...
Resolved: the ticket↔PR linking convention (rely on the ticket tool's native GitHub integration, not a custom scheme), for Linear specifically event ingestion (webhooks) and execution model (inline, for now), failure-comment deduplication, and step 4's clarification back-and-forth (a human reply on an already-refined ticket re-runs the refine agent — see "Linear ticket refine + plan" above) — see [CLAUDE.md](CLAUDE.md#state-derived-not-stored) and [CLAUDE.md](CLAUDE.md#open-design-decisions).
